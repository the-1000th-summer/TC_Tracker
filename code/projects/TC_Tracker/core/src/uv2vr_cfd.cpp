...
bool uv2vr_cfd::calRV(TTCore::TwoDArray &u, TTCore::TwoDArray &v, float *latData, float *lonData, int latSize, int lonSize, float msgValue, int iopt, TTCore::TwoDArray &rv) {
    ...
    for (int lonIndex = mlStart; lonIndex < mlEnd; ++lonIndex) {
        ...
        for (int latIndex = 1; latIndex < latSize-1; ++latIndex) {
            float v_lonP1 = v(latIndex,lonIndexP1), v_lonM1 = v(latIndex,lonIndexM1);
            float u_latP1 = u(latIndex+1,lonIndex), u_latM1 = u(latIndex-1,lonIndex);
            float u_here = u(latIndex,lonIndex);
        ...
        if (jopt < 2) { continue; }
        // bottom bound (nl = 1)
        float v_b_lonP1 = v(0,lonIndexP1), v_b_lonM1 = v(0,lonIndexM1);
        float u_b_latP1 = u(1,lonIndex), u_b_here = u(0,lonIndex);
        ...
        // top bound (nl = nlat)
        float v_t_lonP1 = v(latSize-1,lonIndexP1), v_t_lonM1 = v(latSize-1,lonIndexM1);
        float u_t_here = u(latSize-1,lonIndex), u_t_latM1 = u(latSize-2,lonIndex);
    ...
    if (jopt == 2) {      // left/right bound?
        for (int latIndex = 1; latIndex < latSize - 1; ++latIndex) {
            ...
            float v_l_lonP1 = v(latIndex,1), v_l_here = v(latIndex,0);
            float u_l_latP1 = u(latIndex+1,0), u_l_latM1 = u(latIndex-1,0);
            float u_l_here = u(latIndex,0);
            ...
            float v_r_here = v(latIndex,lonSize-1), v_r_lonM1 = v(latIndex,lonSize-2);
            float u_r_latP1 = u(latIndex+1,lonSize-1), u_r_latM1 = u(latIndex-1,lonSize-1);
            float u_r_here = u(latIndex,lonSize-1);
